...
$(document).ready(function () {
  ...
  if (
    $(".banner_primary").length > 0 &&
    $(".banner_primary").hasClass("banner_fade")
  ) {
    ...
    $.each($(".neo_countdown .neo_countdown_item"), function (index, item) {
      ...
      function countdownBanner() {
        ...
        jQuery({ Counter: 0 }).animate(
          { Counter: $number.attr("data-value") },
          {
            duration: 2000,
            easing: "swing",
            step: function () {
  ...
  $(".nav-tabs a").click(function () {
    ...
    setTimeout(function () {
      tabs_view
        .find(
          $(".tab-content .tab-pane[aria-labelledby=" + tab_id + "] .Carousel")
        )
        .slick("refresh");
    }, 300);
  });
...
function getSpecialPrices() {
  if ($(".login-user-info").length > 0) {
    if ($("#prd_id_discount_url").length > 0) {
      var item_ids_str = "";
      $(".Product")
        .not(".prd_list_container .Product")
      ...
      $.ajax({
        ...
        success: function (msg) {
          for (i in msg) {
            ...
            $(".Product_" + prd_id)
              .find(".discount span")
              .html(parseInt(msg[i].prd_discount));
            $(".Product_" + prd_id)
              .find(".price_sale")
              .html(msg[i].prd_final_price_display);
            showCurrencies();
          }
        },
      });
...
$.fn.loadPageSubmit = function () {
  ...
  $(container).html(
    '<div class="loading-wrapper"><div class="loading"><div></div><div></div><div></div><div></div></div></div>'
  );
...
$.fn.updatePrice = function () {
  ...
  var s = parseFloat(
    $("input[name='ord_cargo_id']:checked").attr("data-price")
  );
  if (!s) s = 0;
  var cs = c + s;
  ...
  if (cso != cs) {
    ...
    if ($(".ord_installment_box").length > 0) {
      ...
      $("#div_intsallments").load(
        https_url +
          "?p=Installments&https=1&bank_id=" +
          $("input[name='ord_bank_id']:checked").val() +
          "&amount=" +
          $("#total_amount").val()
      );
    }
...
$.fn.updateCargoList = function () {
  ...
  $("#cargo_container").load(
    https_url +
      ...
      town_id +
      "&type=" +
      $("input[name='ord_payment_type']:checked").val(),
    "",
    function () {
      $(this).updatePrice();
    }
...
$.fn.inlineScripts = function () {
  ...
  if ($(".actions_list_box").length > 0) {
    var elm = this;
    mod_number = $(".actions_list_box")
      .closest(".mod_container")
      .attr("id")
      .split("_")[2];
...
$.fn.adrChange = function () {
  ...
  if (g) {
    ...
    if (v) {
      $(".show_corp,.show_corp2").show();
    } else {
      $(".show_corp,.show_corp2").hide();
    }
  } else {
  ...
  $.ajax({
    ...
    success: function (msg) {
      ...
      if (msg != "") {
        var slide_index = 1;
        var slide_index =
          $(".cycle-slideshow-whats-happening").data("cycle.opts").currSlide +
          1;

        $(".cycle-slideshow-whats-happening")
          .prepend(msg)
          .cycle("reinit")
          .cycle("goto", slide_index);
...
$.fn.updatePrdView = function () {
  ...
  if ($(".prd_opts").length > 0) {
    ...
    if (opt.data("id")) {
      ...
      $("#prd_final_price_display").html(
        $(opt_data_obj).find(".data-prd-final-price").html()
      );
      $("#prd_special_price_display").html(
        $(opt_data_obj).find(".data-prd-special-price").html()
      );
      $("#prd_price_display").html(
        $(opt_data_obj).find(".data-prd-price").html()
      );
      $("#min_inst_price_display").html(
        $(opt_data_obj).find(".data-prd-min-inst-price").html()
      );
      $("#prd_eft_price_display").html(
        $(opt_data_obj).find(".data-prd-eft-price").html()
      );
      $("#stock_status_display").html(
        opt_stock > 0 ? $("#prd_in_stock").val() : $("#prd_no_stock").val()
      );
    }
  }
};
...
function mypopup(url, w, h) {
  ...
  window.open(
    ...
    "width=" +
      ...
      ",left=" +
      l +
      ",toolbars=no,resizable=yes,scrollbars=yes"
  );
}
        ...
        success: function (res) {
          if (res.length > 0) {
            ...
            res.forEach(function (item) {
              ...
              $(".Search-autocomplete-content").append(
                '<li data-id="' +
                  item.id +
                  '">' +
                  imageData +
